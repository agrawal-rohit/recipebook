...
describe("detectPackageManagerFromLockfileList", () => {
	...
	const absolute = path.join(os.tmpdir(), "cheetos-packages-detect");
...
describe("npmEcosystemAdapter.detectFromManifest", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cheetos-manifest-"));
	});
	afterEach(() => {
...
describe("selectPackageManager", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cheetos-select-pm-"));
	});
	afterEach(() => {
		fs.rmSync(tempDir, { recursive: true, force: true });
	});
