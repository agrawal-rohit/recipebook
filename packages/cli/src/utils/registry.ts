...
export interface LocateRegistryOptions {
	...
	registry?: string;
	/** Registry source persisted via `recipebook configure set`. */
...
export async function locateRegistry(
	...
	const source =
		options.registry ??
		process.env.RECIPEBOOK_REGISTRY ??
		options.savedRegistry;
 ...
 * @param savedRegistry - Optional registry source persisted via `recipebook configure set`.
