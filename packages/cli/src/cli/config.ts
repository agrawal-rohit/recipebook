...
/** Persisted recipebook CLI settings (`registry` = default URL or local path). */
export interface RecipebookConfig {
	registry?: string;
}

/**
 * Return the absolute path to the global recipebook config file.
...
export function configPath(env: NodeJS.ProcessEnv = process.env): string {
	...
	return path.join(base, "recipebook", "config.json");
}

/**
 * Narrow parsed JSON to the known recipebook config shape.
 ...
 */
function recipebookConfigFromJson(parsed: unknown): RecipebookConfig {
 ...
 */
function validatedConfig(config: { registry?: unknown }): RecipebookConfig {
...
function assertConfigPathIsRegularFile(
	...
	if (stat.isSymbolicLink())
		throw new Error(
			`Cannot ${action} recipebook config at ${filePath}: file is a symbolic link.`,
		);
	if (stat.isDirectory())
		throw new Error(
			`Cannot ${action} recipebook config at ${filePath}: path is a directory.`,
		);
	if (!stat.isFile())
		throw new Error(
			`Cannot ${action} recipebook config at ${filePath}: path is neither a file nor a directory.`,
		);
}
...
async function assertReadableConfigFile(filePath: string): Promise<boolean> {
	...
	if (stat.size > CONFIG_FILE_BYTE_LIMIT)
		throw new Error(
			`Cannot read recipebook config at ${filePath}: file is too large.`,
		);
	return true;
}

/**
 * Read the global recipebook config.
...
export async function readConfig(
	...
	try {
		return recipebookConfigFromJson(JSON.parse(raw) as unknown);
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		throw new Error(
			`Malformed recipebook config at ${filePath}: ${message}. Fix or delete the file, then retry.`,
		);
	}
}

/**
 * Write the global recipebook config, creating parent directories as needed.
